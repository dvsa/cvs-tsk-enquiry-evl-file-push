...
import { configureTflFile } from '../../src/fileConvert/fileConvert';
...
describe('test the file config', () => {
  ...
  test('expect filename to be correct', async () => {
    await configureTflFile('', buffer, 'EVL_GVT_20220621.csv');
...
describe('test the file config failure condition', () => {
  test('no data provided throws an error', async () => {
    await expect(
      configureTflFile('', Buffer.from(''), 'EVL_GVT_20220621.csv'),

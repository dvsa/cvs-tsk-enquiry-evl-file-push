...
export const configureFile = async (workingDir: string, bufferInput: Buffer) => {
  ...
  const formattedDate = new Date()
    ...
    .reverse()
    .join('');
  const zipCsvFilename = workingDir + archiveNamePrefix + formattedDate + '.csv.gz';
  const textFilename = workingDir + textFilenamePrefix + formattedDate + '.txt';
  const archiveName = workingDir + archiveNamePrefix + formattedDate + '.tar.gz';
